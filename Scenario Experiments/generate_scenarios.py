...
print(f"Generating {num_scenarios} autoregressive scenarios...")
scenarios = []
...
for s in range(num_scenarios):
    ...
    for h in range(horizon):
        ...
        p = np.clip(np.random.normal(loc=0.5, scale=0.15 * temperature), 0.01, 0.99)
...
plt.title("Chronos-2: Native Multistep vs Autoregressive Scenarios")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig('Scenario Experiments/scenarios_comparison.png')
print("Saved plot to Scenario Experiments/scenarios_comparison.png")
