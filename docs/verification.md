...
| `n_forecast` | hours the run predicted in the bucket. Below `n` never; above it exactly when the gauge had a gap |
...
pool(scores)                               # by lead bucket
pool(scores, by=[])                        # one row for everything
pool(scores, by=["model_id", "lead_lo"])   # eras kept apart
```
...
and knows that RMSE averages in squares; a plain `groupby().mean()` gets both wrong.
...
Replay backtests of the same checkpoint score MAE 0.441 against live's 0.423. That gap is
the price of an older weather snapshot, and it is the right sign — a replay should be
slightly more pessimistic than the run it reconstructs, never more flattering.
